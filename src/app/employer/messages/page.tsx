...
import { MessageSquareDashed, MessageCircle, UserCheck, Info, Loader2 } from "lucide-react";
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
            <div>
              <CardTitle className="text-2xl font-bold text-primary flex items-center">
                <MessageCircle className="mr-2 h-7 w-7" /> Candidate Conversations
              </CardTitle>
              <CardDescription>
                Engage with matched candidates securely and manage your hiring pipeline.
              </CardDescription>
            </div>
             <Button className="mt-3 sm:mt-0" onClick={() => router.push('/employer/jobs')}>
              Manage Job Postings
            </Button>
          </div>
        </CardHeader>
           ...
           {activeChats && activeChats.length === 0 ? (
            <div className="p-10 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                When candidates express interest or you initiate contact, conversations will appear here.
              </p>
            </div>
          ) : (
                ...
                <li key={chat.id} className="p-4 hover:bg-muted/50 cursor-pointer transition-colors" onClick={() => router.push(`/messages/${chat.id}`)}>
                  <div className="flex items-center gap-4">
                    <Avatar className="h-12 w-12">
                      <AvatarImage src={`https://picsum.photos/seed/${chat.participantIds?.find(id => id !== authUser?.id)}/100`} alt="Candidate" data-ai-hint="abstract user"/>
                      <AvatarFallback><UserCheck className="h-6 w-6"/></AvatarFallback>
                    </Avatar>
                    <div className="flex-grow overflow-hidden">
                      <h3 className="font-semibold text-primary truncate">
                        {chat.candidateProfileSnapshot?.anonymizedName || 'Anonymous Candidate'} for {chat.jobPostingSnapshot?.title}
                      </h3>
         ...
         {activeChats && activeChats.length > 0 && (
          <CardFooter className="border-t p-4 bg-secondary/50">
             <div className="flex items-start text-xs text-muted-foreground">
                ...
                <div>
                    <p className="font-semibold">Managing Candidate Conversations:</p>
                    <ul className="list-disc list-inside pl-1">
                        <li>Conversations are initiated based on mutual interest for a specific job posting.</li>
                        <li>Reveal Tiers control the amount of information shared. Progress to higher tiers cautiously and with candidate consent.</li>
                        <li>Small engagement fees may apply to unlock conversations and progress to higher tiers, ensuring platform sustainability and meaningful interactions.</li>
                    </ul>
                </div>
             </div>
          </CardFooter>
